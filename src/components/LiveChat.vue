...
const currentView  = ref('chat'); // 'chat' | 'recent'
...
/* ── dropdown actions ── */
const startNewChat = () => {
  recentMessages.value = [...messages.value];
...
const viewRecentChat = () => {
  if (!recentMessages.value.length) recentMessages.value = [...messages.value];
    ...
    <Transition name="chat-pop">
      ...
      >
        ...
        <div class="bg-[#003854] text-white px-4 py-3 flex items-center gap-3 relative shrink-0">
          <!-- Back arrow (recent view only) -->
          <button
            v-if="currentView === 'recent'"
            @click="currentView = 'chat'"
            class="text-white hover:text-gray-300 transition-colors mr-1"
          >
          ...
          <span class="font-semibold font-start flex-1">
            {{ currentView === 'recent' ? 'Recent chat' : 'Chat with support' }}
          </span>
          ...
          <Transition name="dropdown">
            ...
            >
              ...
              <button
                @click="viewRecentChat"
                class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-[#131313] hover:bg-gray-50 transition-colors"
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                <path d="M9 1.5C13.1421 1.5 16.5 4.85786 16.5 9C16.5 13.1421 13.1421 16.5 9 16.5C4.85786 16.5 1.5 13.1421 1.5 9H3C3 12.3137 5.68629 15 9 15C12.3137 15 15 12.3137 15 9C15 5.68629 12.3137 3 9 3C6.93766 3 5.11835 4.0405 4.03838 5.62518L6 5.625V7.125H1.5V2.625H3L2.99992 4.49938C4.36825 2.67806 6.54655 1.5 9 1.5ZM9.75 5.25L9.74985 8.68875L12.1819 11.1213L11.1213 12.1819L8.24985 9.30975L8.25 5.25H9.75Z" fill="black"/>
              </svg>
                View recent chat
              </button>
            </div>
        ...
        <div ref="messagesEl" class="flex-1 overflow-y-auto px-4 py-4 space-y-3 bg-white">
          <template v-for="(msg, i) in (currentView === 'recent' ? recentMessages : messages)" :key="i">
            <!-- Assistant -->
        ...
        <div v-if="currentView === 'chat'" class="border-t border-gray-100 px-3 py-3 flex items-center gap-2 shrink-0">
          <input
            v-model="inputText"
